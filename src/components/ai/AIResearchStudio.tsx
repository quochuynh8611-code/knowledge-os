    ...
    <div className="bg-white border border-stone-200/90 rounded-2xl shadow-xl flex flex-col h-full max-h-[85vh] overflow-hidden">
      ...
      <div className="p-4 md:px-6 md:py-4 bg-gradient-to-r from-amber-900 to-stone-900 text-stone-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-amber-500/20 border border-amber-400/30 text-amber-300 flex items-center justify-center font-bold shrink-0">
      ...
      <div className="p-4 border-b border-stone-200 bg-stone-50/80 space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            <label
              ...
              className="block text-[11px] font-bold uppercase tracking-wider text-stone-600 mb-1"
            >
        ...
        <div className="pt-2 border-t border-stone-200/70 grid grid-cols-1 md:grid-cols-12 gap-3 text-xs">
          ...
          <div className="md:col-span-6 space-y-1.5">
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-stone-800">
              ...
              {sourceScope.obsidianVault && (
                <button
                  ...
                  className="px-2 py-0.5 rounded-lg bg-purple-100 hover:bg-purple-200 text-purple-800 border border-purple-300 text-[11px] font-semibold flex items-center gap-1 transition cursor-pointer"
                >
              ...
              <label
                className="flex items-center gap-1.5 text-stone-400 cursor-not-allowed select-none"
              ...
              >
                <input
                  ...
                  checked={false}
                  disabled={true}
                  className="rounded text-stone-300 cursor-not-allowed"
                />
            ...
            {sourceScope.obsidianVault && selectedObsidianPaths.length > 0 && (
              <div className="mt-2 p-2.5 bg-purple-50/70 border border-purple-200/80 rounded-xl text-xs text-purple-950 space-y-1">
                <div className="flex items-center justify-between font-semibold">
                  ...
                  <button
                    type="button"
                    onClick={() => setShowObsidianPicker(true)}
                    className="text-[11px] text-purple-700 underline hover:text-purple-900 cursor-pointer"
                  >
                    Đổi tài liệu
                  </button>
                </div>
                <ul className="list-disc list-inside text-[11px] text-purple-900 space-y-0.5 font-mono">
                  {selectedObsidianPaths.map((p) => (
          ...
          <div className="md:col-span-3 space-y-1.5">
            ...
            <select
              ...
              className="w-full text-xs font-medium bg-white border border-stone-300 rounded-lg px-2.5 py-1.5 text-stone-900 focus:ring-2 focus:ring-amber-700 disabled:opacity-50"
            >
                ...
                {topicSessions.map((s) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`text-[10px] ${
                          isSelected ? 'text-amber-200' : 'text-stone-400 font-mono'
                        }`}
                      >
                        {formattedTime}
                      </span>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-stone-50/50 space-y-4">
        ...
        {fallbackWarning && (
          <div
            role="status"
            className="p-3 bg-amber-50 border border-amber-200 text-amber-900 rounded-xl text-xs flex items-start gap-2"
          >
          ...
          <div className="space-y-4">
            ...
            {proposedOutline && proposedOutline.length > 0 && (
              <div className="bg-amber-50/70 border border-amber-200/80 rounded-xl p-3.5 text-xs text-amber-950 space-y-2">
                ...
                <ul className="list-decimal list-inside space-y-1 pl-1 text-stone-800">
                  {proposedOutline.map((item, idx) => (
                    <li key={idx}>
                      <span className="font-semibold">{item.title}</span>
                      {item.description && (
                        <span className="text-stone-600 text-[11px]"> — {item.description}</span>
                      )}
                    </li>
                  ))}
                </ul>
            ...
            {citations && citations.length > 0 && (
              <div className="bg-white border border-stone-200 rounded-2xl p-4 shadow-xs space-y-2.5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
                  {citations.map((cit) => {
                    ...
                    return (
                      <div
                        key={cit.id}
                        className="p-2.5 bg-stone-50 border border-stone-200 rounded-xl space-y-1"
                      >
                        <div className="flex items-center justify-between gap-2">
                          <span className="font-mono text-[10px] font-bold text-amber-900 bg-amber-100/70 px-1.5 py-0.5 rounded">
                        ...
                        <p className="font-semibold text-stone-900">{cit.sourceTitle}</p>
            ...
            {uncertainties && uncertainties.length > 0 && (
              <div className="bg-amber-50/60 border border-amber-200 rounded-2xl p-4 shadow-xs space-y-2">
                ...
                <div className="space-y-1.5 text-xs text-stone-800">
                  {uncertainties.map((unc, idx) => (
                    <div key={idx} className="p-2 bg-white/80 rounded-lg border border-amber-200/60">
                      <span className="font-bold text-amber-950">{unc.point}:</span>{' '}
                      <span className="text-stone-700">{unc.reason}</span>
                    </div>
                  ))}
                </div>
              </div>
