...
export function Navbar() {
  ...
  return (
    <>
      <header className="sticky top-0 z-30 bg-stone-50/90 backdrop-blur-md border-b border-stone-200/80 px-4 lg:px-6 py-3">
        <div className="flex items-center justify-between gap-4 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-stone-900 text-sm sm:text-base tracking-tight leading-tight flex items-center gap-2">
                Nghiên Cứu Phật Học &amp; Huyền Học
                <span className="hidden sm:inline text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 bg-amber-100 text-amber-900 rounded-full border border-amber-300">
                  Knowledge OS
                </span>
              </h1>
              <p className="text-[11px] text-stone-500 hidden sm:block">
                Hệ thống hóa Abhidharma, Tam Tạng, Thiền định &amp; Huyền học phương Đông
              </p>
            </div>
          </div>
          ...
          <div className="flex-1 max-w-md hidden md:block">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-1.5 text-xs bg-white border border-stone-300 rounded-xl text-stone-900 placeholder:text-stone-400 focus:outline-hidden focus:ring-2 focus:ring-amber-700/50 transition"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {reviewQueue.length > 0 && (
              <button
                ...
                className="relative flex items-center gap-1.5 px-3 py-1.5 bg-amber-100 hover:bg-amber-200 text-amber-900 rounded-xl text-xs font-semibold border border-amber-300 transition"
            ...
            >
              <Timer className={`w-3.5 h-3.5 ${isTimerRunning ? 'text-emerald-700 animate-spin' : 'text-stone-500'}`} />
              <span className="font-mono">{formatTimerMinSec(timerSeconds)}</span>
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 bg-purple-50 hover:bg-purple-100 text-purple-900 border border-purple-200/80 rounded-xl text-xs font-semibold transition"
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-900 border border-blue-200/80 rounded-xl text-xs font-semibold transition"
            ...
            <button
              ...
              className="p-2 text-stone-600 hover:text-stone-900 hover:bg-stone-200/60 rounded-xl transition"
            ...
            <div className="relative">
              ...
              {showQuickAddMenu && (
                <>
                  <div className="fixed inset-0 z-40" onClick={() => setShowQuickAddMenu(false)} />
      ...
      {/* Global Modals */}
      <TopicFormModal isOpen={showTopicModal} onClose={() => setShowTopicModal(false)} />
      <NoteFormModal isOpen={showNoteModal} onClose={() => setShowNoteModal(false)} />
      <ResourceFormModal isOpen={showResourceModal} onClose={() => setShowResourceModal(false)} />
      <ExportImportModal isOpen={showExportModal} onClose={() => setShowExportModal(false)} />
      <ObsidianBridgeModal isOpen={showObsidianModal} onClose={() => setShowObsidianModal(false)} />
      <NotebookLMStudioModal isOpen={showNotebookLMModal} onClose={() => setShowNotebookLMModal(false)} />
