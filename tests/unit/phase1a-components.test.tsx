...
import { TopicSkeleton } from "../../src/components/ui/TopicSkeleton";
import { NoteSkeleton } from "../../src/components/ui/NoteSkeleton";
  ...
  describe("LoadingSkeleton, TopicSkeleton & NoteSkeleton", () => {
    ...
    it("TopicSkeleton render đúng ở cả 3 chế độ card, detail, tree", () => {
      const { container: cardContainer } = render(
        <TopicSkeleton mode="card" count={2} />,
      );
      expect(
        cardContainer.querySelectorAll(".animate-pulse").length,
      ).toBeGreaterThan(0);

      const { container: detailContainer } = render(
        <TopicSkeleton mode="detail" />,
      );
      expect(detailContainer.querySelector(".max-w-5xl")).toBeInTheDocument();

      const { container: treeContainer } = render(
        <TopicSkeleton mode="tree" count={3} />,
      );
    ...
    it("Render đầy đủ tiêu đề, mô tả, nút CTA chính và nút phụ", () => {
      ...
      render(
        <EmptyState
          ...
          suggestions={[{ label: "Abhidharma", onClick: handleSuggestion }]}
        />,
      );

      expect(screen.getByText("Chưa có chủ đề nào")).toBeInTheDocument();
      expect(
        screen.getByText("Hãy tạo chủ đề khảo cứu đầu tiên của bạn"),
      ).toBeInTheDocument();
